...
# Mostrar el mensaje con un ancho más grueso y color verde.
...
# Imprimir mensaje de advertencia de copia de seguridad.
printf "${ORANGE}$(tput smso)¡HAGA UNA COPIA DE SEGURIDAD DE SUS ARCHIVOS ANTES DE CONTINUAR!$(tput rmso)\n"
printf "\n"
printf "\n"
sleep 2
...
printf "\n%s - Se necesita Yay, es el ayudante de AUR para instalar algunos paquetes. Verificando si se encuentra instalado...\n" "${NOTE}"
sleep 2
ISAUR=$(command -v yay)
...
for PKG1 in foot swaybg swaylock-effects wofi wlogout mako grim slurp wl-clipboard cliphist swappy polkit-kde-agent nwg-look-bin swww mousepad pipewire-alsa playerctl; do
...
read -n1 -rep "${CAT} OPCIONAL - ¿Le gustaría instalar Thunar como administrador de archivos? (s/n)" inst3
echo
...
# Verificar las carpetas de configuración existentes y la copia de seguridad. 
    for DIR in btop cava foot hypr mako swappy swaylock waybar wlogout wofi 
    do 
        DIRPATH=~/.config/$DIR
        if [ -d "$DIRPATH" ]; then 
    ...
    do 
        ...
        if [ -d "$DIRPATH" ]; then 
            echo -e "${NOTE} - wallpapers en $DIRw encontrado, intentando hacer una copia de seguridad."
            mv $DIRPATH $DIRPATH-back-up 2>&1 | tee -a "$LOG"
    ...
    cp -r accesorios/Fondos ~/Imágenes/ && { echo "¡Copia completada!"; } || { echo "Error: no se pudieron copiar los fondos de pantalla"; exit 1; } 2>&1 | tee -a "$LOG"
